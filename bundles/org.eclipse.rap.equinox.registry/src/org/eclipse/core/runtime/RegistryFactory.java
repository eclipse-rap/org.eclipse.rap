...
//                request/session aware translation mechanism. To allow this the
//                following class extensions are used.
    private static Runnable translatorAlgorithm;
    public static ThreadLocal algorithmIO = new ThreadLocal();
    
	public static void setTranslatorAlgorithm( final Runnable algorithm ) {
    ...
    {
        String result = key;
        if( key != null && translatorAlgorithm != null ) {
          String translation = null;
          if( translatorAlgorithm != null ) {
            RegistryFactory.algorithmIO.set( new String[] {
               key,
               symbolicName
            } );
            try {
              translatorAlgorithm.run();
              translation = ( String )RegistryFactory.algorithmIO.get();
            } finally {
              RegistryFactory.algorithmIO.set( null );
            }
          }
          result = translation == null ? key : translation;
        }
        return result;
    }
}
