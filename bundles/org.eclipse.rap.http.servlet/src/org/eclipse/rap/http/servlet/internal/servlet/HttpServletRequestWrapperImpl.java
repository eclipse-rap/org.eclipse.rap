...
public class HttpServletRequestWrapperImpl extends HttpServletRequestWrapper {
	...
	public HttpSession getSession(boolean create) {
		...
		if (session != null) {
			...
			return currentDispatchTarget.getContextController().getSessionAdaptor(session,
					currentDispatchTarget.getServletRegistration().getT().getServletConfig().getServletContext());
		}
	...
	public void removeAttribute(String name) {
		...
		ServletRequestAttributeEvent servletRequestAttributeEvent = new ServletRequestAttributeEvent(
	...
	private List<Part> getParts0() throws IOException, ServletException {
		org.eclipse.rap.http.servlet.internal.registration.ServletRegistration servletRegistration = getServletRegistration();
	...
	private org.eclipse.rap.http.servlet.internal.registration.ServletRegistration getServletRegistration() {
		EndpointRegistration<?> servletRegistration = dispatchTargets.peek().getServletRegistration();

		if (servletRegistration instanceof org.eclipse.rap.http.servlet.internal.registration.ServletRegistration) {
			return (org.eclipse.rap.http.servlet.internal.registration.ServletRegistration) servletRegistration;
		}
