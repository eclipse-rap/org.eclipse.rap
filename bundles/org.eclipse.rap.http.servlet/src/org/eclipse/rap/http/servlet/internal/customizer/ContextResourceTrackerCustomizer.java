...
public class ContextResourceTrackerCustomizer
	...
	public ContextResourceTrackerCustomizer(BundleContext bundleContext, HttpServiceRuntimeImpl httpServiceRuntime,
		...
		try {
			...
			if (!contextController.matches(serviceReference)) {
				...
				throw new HttpWhiteboardFailureException(
						"Whiteboard resources cannot bind to legacy contexts. " + serviceReference, //$NON-NLS-1$
						DTOConstants.FAILURE_REASON_NO_SERVLET_CONTEXT_MATCHING);
			}
	...
	private void recordFailed(ServiceReference<Object> serviceReference, int failureReason) {
		...
		failedResourceDTO.patterns = StringPlus
				.from(serviceReference.getProperty(HttpWhiteboardConstants.HTTP_WHITEBOARD_RESOURCE_PATTERN))
				.toArray(new String[0]);
		failedResourceDTO.prefix = String
				.valueOf(serviceReference.getProperty(HttpWhiteboardConstants.HTTP_WHITEBOARD_RESOURCE_PREFIX));
