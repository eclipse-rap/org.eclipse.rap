...
public class CBanner extends Composite {
  ...
  Control right;
  Control bottom;
  /** Sash to replace the curved separator in RWT */
  Sash separator;
  boolean simple = true;
  int[] curve;
  int curveStart = 0;
  ...
  // static int BORDER1 = RWT.COLOR_WIDGET_HIGHLIGHT_SHADOW;
  /**
   * Constructs a new instance of this class given its parent and a style value
   * describing its behavior and appearance.
   * <p>
   * The style value is either one of the style constants defined in class
   * <code>RWT</code> which is applicable to instances of this class, or must
   * be built by <em>bitwise OR</em>'ing together (that is, using the
   * <code>int</code> "|" operator) two or more of those <code>RWT</code>
  ...
  public CBanner( Composite parent, int style ) {
    ...
    // }
    // };
    // int[] events = new int[] {RWT.Dispose, RWT.MouseDown, RWT.MouseExit,
    // RWT.MouseMove, RWT.MouseUp, RWT.Paint, RWT.Resize};
  ...
  public int getRightWidth() {
    ...
    if( right == null )
      return 0;
    if( rightWidth == RWT.DEFAULT ) {
      Point size = right.computeSize( RWT.DEFAULT, RWT.DEFAULT, false );
      return size.x;
    }
    return rightWidth;
  }
   ...
   * rightDragDisplacement); if (rightMinWidth == RWT.DEFAULT) { Point minSize =
   ...
   * getSize(); //
   * gc.setBackground(getDisplay().getSystemColor(RWT.COLOR_GREEN)); //
   ...
   *              values of size are less than RWT.DEFAULT</li>
   *              </ul>
   * @since 3.1
   */
  public void setRightMinimumSize( Point size ) {
    // checkWidget();
    if( size == null || size.x < RWT.DEFAULT || size.y < RWT.DEFAULT )
      RWT.error( RWT.ERROR_INVALID_ARGUMENT );
