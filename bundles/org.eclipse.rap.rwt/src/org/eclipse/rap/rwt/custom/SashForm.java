...
public SashForm( final Composite parent , final int style ) {
  super( parent, style );
	...
	sashStyle = ((style & RWT.VERTICAL) != 0) ? RWT.HORIZONTAL : RWT.VERTICAL;
...
void onDragSash( final SelectionEvent event ) {
	...
	if (getOrientation() == RWT.HORIZONTAL) {
...
//	checkWidget();
	...
	sashStyle &= ~(RWT.HORIZONTAL | RWT.VERTICAL);
	sashStyle |= orientation == RWT.VERTICAL ? RWT.HORIZONTAL : RWT.VERTICAL;
	for (int i = 0; i < sashes.length; i++) {
//		sashes[i].dispose();
