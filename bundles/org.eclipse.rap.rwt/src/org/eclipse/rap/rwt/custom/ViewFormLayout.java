...
class ViewFormLayout extends Layout {
                               ...
                               boolean flushCache )
  {
    ...
    if( left != null ) {
      leftSize = computeChildSize( left, RWT.DEFAULT, RWT.DEFAULT, flushCache );
    }
    ...
    if( right != null ) {
      rightSize = computeChildSize( right, RWT.DEFAULT, RWT.DEFAULT, flushCache );
    }
    Point size = new Point( 0, 0 );
    // calculate width of title bar
    if( form.separateTopCenter
        || ( wHint != RWT.DEFAULT && leftSize.x + centerSize.x + rightSize.x > wHint ) )
    {
  ...
  protected void layout( Composite composite, boolean flushCache ) {
    ...
    if( form.separateTopCenter || minTopWidth > rect.width ) {
      ...
      if( left != null && !left.isDisposed() ) {
        ...
        leftSize = computeChildSize( left, leftW, RWT.DEFAULT, false );
      ...
      if( center != null && !center.isDisposed() ) {
        ...
        centerSize = computeChildSize( center, w, RWT.DEFAULT, false );
