...
class SashFormLayout extends Layout {
                               ...
                               boolean flushCache )
  {
    ...
    boolean vertical = sashForm.getOrientation() == RWT.VERTICAL;
    int maxIndex = 0;
    int maxValue = 0;
    for( int i = 0; i < cArray.length; i++ ) {
      if( vertical ) {
        Point size = cArray[ i ].computeSize( wHint, RWT.DEFAULT, flushCache );
        if( size.y > maxValue ) {
          maxIndex = i;
          maxValue = size.y;
        }
        ...
        Point size = cArray[ i ].computeSize( RWT.DEFAULT, hHint, flushCache );
        if( size.x > maxValue ) {
          maxIndex = i;
          maxValue = size.x;
        }
    ...
    if( ratios[ maxIndex ] > 0 ) {
      int sashwidth = sashForm.sashes.length > 0
                                                ? sashForm.SASH_WIDTH
                                                  + sashForm.sashes[ 0 ].getBorderWidth()
                                                  * 2
                                                : sashForm.SASH_WIDTH;
      if( vertical ) {
  ...
  protected void layout( Composite composite, boolean flushCache ) {
    ...
    if( sashForm.sashes.length < controls.length - 1 ) {
      ...
      System.arraycopy( sashForm.sashes,
                        0,
                        newSashes,
                        0,
                        sashForm.sashes.length );
    ...
    int sashwidth = sashes.length > 0
                                     ? sashForm.SASH_WIDTH
                                       + sashes[ 0 ].getBorderWidth()
                                       * 2
                                     : sashForm.SASH_WIDTH;
    if( sashForm.getOrientation() == RWT.HORIZONTAL ) {
      ...
      if( controls.length > 1 ) {
        sashes[ sashes.length - 1 ].setBounds( x,
                                               area.y,
                                               sashwidth,
                                               area.height );
        x += sashwidth;
        width = area.width - x;
        controls[ controls.length - 1 ].setBounds( x,
                                                   area.y,
                                                   width,
                                                   area.height );
      }
    } else {
      int height = ( int )( ratios[ 0 ]
                            * ( area.height - sashes.length * sashwidth ) / total );
      int y = area.y;
      ...
      if( controls.length > 1 ) {
        ...
        controls[ controls.length - 1 ].setBounds( area.x,
                                                   y,
                                                   area.width,
                                                   height );
      }
