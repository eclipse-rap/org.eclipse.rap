...
public FormAttachment (int numerator, int denominator, int offset) {
	if (denominator == 0) RWT.error (RWT.ERROR_CANNOT_BE_ZERO);
...
public FormAttachment (Control control, int offset) {
	this (control, offset, RWT.DEFAULT);
}
...
int solveY (int value) {
	if (numerator == 0) RWT.error (RWT.ERROR_CANNOT_BE_ZERO);
