...
protected Point computeSize (Composite composite, int wHint, int hHint, boolean flushCache) {
	...
	if (wHint != RWT.DEFAULT) size.x = wHint;
	if (hHint != RWT.DEFAULT) size.y = hHint;
	return size;
}
...
Point layout (Composite composite, boolean move, int x, int y, int width, int height, boolean flushCache) {
	...
	for (int i=0; i<count; i++) {
		...
		if (data.grabExcessHorizontalSpace && data.minimumWidth > 0) {
			if (data.cacheWidth < data.minimumWidth) {
				...
				data.cacheWidth = data.cacheHeight = RWT.DEFAULT;
		...
		for (int i=0; i<rowCount; i++) {
			...
			if (data != null) {
				...
				if (hSpan == 1) {
					...
					if (!data.grabExcessHorizontalSpace || data.minimumWidth != 0) {
						w = !data.grabExcessHorizontalSpace || data.minimumWidth == RWT.DEFAULT ? data.cacheWidth : data.minimumWidth;
	...
	if (makeColumnsEqualWidth) {
		...
		columnWidth = width == RWT.DEFAULT || expandCount == 0 ? columnWidth : Math.max (minColumnWidth, availableWidth / columnCount);
		for (int i=0; i<columnCount; i++) {
			expandColumn [i] = expandCount > 0;
			widths [i] = columnWidth;
		}
	} else {
		if (width != RWT.DEFAULT && expandCount > 0) {
			int totalWidth = 0;
			for (int i=0; i<columnCount; i++) {
				totalWidth += widths [i];
			}
						...
						if (data != null) {
							...
							if (hSpan > 1) {
								if (!data.grabExcessHorizontalSpace || data.minimumWidth != 0) {
									...
									int w = !data.grabExcessHorizontalSpace || data.minimumWidth == RWT.DEFAULT ? data.cacheWidth : data.minimumWidth;
	...
	if (width != RWT.DEFAULT) {
		for (int j=0; j<columnCount; j++) {
			for (int i=0; i<rowCount; i++) {
				GridData data = getData (grid, i, j, rowCount, columnCount, false);
				if (data != null) {
					if (data.heightHint == RWT.DEFAULT) {
						...
						if ((currentWidth != data.cacheWidth && data.horizontalAlignment == RWT.FILL) || (data.cacheWidth > currentWidth)) {
							int trim = 0;
							if (child instanceof Scrollable) {
								Rectangle rect = ((Scrollable) child).computeTrim (0, 0, 0, 0);
								trim = rect.width;
		...
		for (int j=0; j<columnCount; j++) {
			...
			if (data != null) {
				...
				if (vSpan == 1) {
					...
					if (!data.grabExcessVerticalSpace || data.minimumHeight != 0) {
						h = !data.grabExcessVerticalSpace || data.minimumHeight == RWT.DEFAULT ? data.cacheHeight : data.minimumHeight;
	...
	if (height != RWT.DEFAULT && expandCount > 0) {
		int totalHeight = 0;
		for (int i=0; i<rowCount; i++) {
					...
					if (data != null) {
						...
						if (vSpan > 1) {
							if (!data.grabExcessVerticalSpace || data.minimumHeight != 0) {
								...
								int h = !data.grabExcessVerticalSpace || data.minimumHeight == RWT.DEFAULT ? data.cacheHeight : data.minimumHeight;
