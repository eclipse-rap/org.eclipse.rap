...
public final class GridLayout extends Layout {
   ...
   * postion of any controls. The default value is 1.
   */
                ...
                boolean flushCache )
  {
    ...
    if( makeColumnsEqualWidth ) {
      ...
      columnWidth = width == RWT.DEFAULT || expandCount == 0
                                                            ? columnWidth
          ...
          if( data != null ) {
            if( data.heightHint == RWT.DEFAULT ) {
              ...
              currentWidth += ( hSpan - 1 )
                              * horizontalSpacing
                              - data.horizontalIndent;
              if( ( currentWidth != data.cacheWidth && data.horizontalAlignment == RWT.FILL )
                  || ( data.cacheWidth > currentWidth ) )
              {
                int trim = 0;
                if( child instanceof Scrollable ) {
                  Rectangle rect = ( ( Scrollable )child ).computeTrim( 0,
                                                                        0,
                                                                        0,
                                                                        0 );
                  trim = rect.width;
          ...
          if( vSpan == 1 ) {
            ...
            if( !data.grabExcessVerticalSpace || data.minimumHeight != 0 ) {
              h = !data.grabExcessVerticalSpace
                  || data.minimumHeight == RWT.DEFAULT
                                                      ? data.cacheHeight
                                                      : data.minimumHeight;
