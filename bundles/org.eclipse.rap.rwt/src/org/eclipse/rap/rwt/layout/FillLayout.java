...
protected Point computeSize (Composite composite, int wHint, int hHint, boolean flushCache) {
	...
	for (int i=0; i<count; i++) {
		...
		int w = wHint, h = hHint;
		if (count > 0) {
			if (type == RWT.HORIZONTAL && wHint != RWT.DEFAULT) {
				w = Math.max (0, (wHint - (count - 1) * spacing) / count);
			}
			if (type == RWT.VERTICAL && hHint != RWT.DEFAULT) {
...
Point computeChildSize (Control control, int wHint, int hHint, boolean flushCache) {
	...
	if (wHint == RWT.DEFAULT && hHint == RWT.DEFAULT) {
		...
		int w = wHint == RWT.DEFAULT ? wHint : Math.max (0, wHint - trimX);
		int h = hHint == RWT.DEFAULT ? hHint : Math.max (0, hHint - trimY);
...
public String toString () {
 	...
 	string += "type="+((type == RWT.VERTICAL) ? "SWT.VERTICAL" : "SWT.HORIZONTAL")+" ";
