...
public final class RowLayout extends Layout {
	 ...
	 * @since 2.0
	 */
	public int type = RWT.HORIZONTAL;
...
protected Point computeSize (Composite composite, int wHint, int hHint, boolean flushCache) {
	Point extent;
	if (type == RWT.HORIZONTAL) {
		extent = layoutHorizontal (composite, false, (wHint != RWT.DEFAULT) && wrap, wHint, flushCache);
	} else {
		extent = layoutVertical (composite, false, (hHint != RWT.DEFAULT) && wrap, hHint, flushCache);
	}
	if (wHint != RWT.DEFAULT) extent.x = wHint;
	if (hHint != RWT.DEFAULT) extent.y = hHint;
	return extent;
}

Point computeSize (Control control, boolean flushCache) {
	int wHint = RWT.DEFAULT, hHint = RWT.DEFAULT;
...
public String toString () {
 	...
 	string += "type="+((type != RWT.HORIZONTAL) ? "RWT.VERTICAL" : "RWT.HORIZONTAL")+" ";
