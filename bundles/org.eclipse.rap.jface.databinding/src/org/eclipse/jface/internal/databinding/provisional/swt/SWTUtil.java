 ...
 * 
 */
// RAP [fappel]: use SessionSingletonBase to be session aware
public class SWTUtil extends SessionSingletonBase{
    /**
     * Stores a work queue for each display
     */
    // RAP [fappel]: change to instance variable to be session aware
    private Map mapDisplayOntoWorkQueue = new HashMap();

    // RAP [fappel]: use SessionSingletonBase to be session aware
    private static SWTUtil getInstance() {
      return ( SWTUtil )getInstance( SWTUtil.class );
    }
    ...
    private static WorkQueue getQueueFor(final Display d) {
        ...
        synchronized (getInstance().mapDisplayOntoWorkQueue) {
            ...
            result = (WorkQueue) getInstance().mapDisplayOntoWorkQueue.get(d);
...
// RAP [fappel]: Display#disposeExec(Runnable) not supported
