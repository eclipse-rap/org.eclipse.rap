...
public class StatusDialogManagerTest extends TestCase {
	...
	public void testCheckingForMessageDuplication1() {
		...
		assertEquals(WorkbenchMessages.get().WorkbenchStatusDialog_SeeDetails,
...
//	}
	...
	public void testDetails1() {
		...
		for (int i = 0; i < 2; i++) {
			...
			assertEquals(IDialogConstants.get().SHOW_DETAILS_LABEL, detailsButton
					.getText());
			...
			assertEquals(IDialogConstants.get().HIDE_DETAILS_LABEL, detailsButton
					.getText());
			assertNotNull(details[0]);
			assertFalse(details[0].isDisposed());
